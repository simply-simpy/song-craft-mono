...
describe("projects routes", () => {
  ...
  const server = createTestServer({ register: projectsRoutes, container: { projectService }, decorateSuperUser: true, user: { clerkId: "user_123", globalRole: GlobalRole.SUPER_ADMIN } });
  ...
  it("GET /projects returns list with pagination", async () => {
    ...
    const res = await server.inject({ method: "GET", url: "/projects?page=1&limit=10&sort=updatedAt&order=desc" });
  ...
  it("GET /projects/:id returns a project", async () => {
    ...
    const res = await server.inject({ method: "GET", url: `/projects/${exampleProject.id}` });
    expect(res.statusCode).toBe(200);
  ...
  it("GET /projects/:id returns 404 when missing", async () => {
    ...
    const res = await server.inject({ method: "GET", url: "/projects/123e4567-e89b-12d3-a456-426614174999" });
  ...
  it("PUT /projects/:id updates a project", async () => {
    projectService.updateProject.mockResolvedValue({ ...exampleProject, name: "Updated" });
  ...
  it("DELETE /projects/:id deletes a project", async () => {
    ...
    const res = await server.inject({ method: "DELETE", url: `/projects/${exampleProject.id}` });
  ...
  it("GET /sessions returns list with pagination", async () => {
    ...
    const res = await server.inject({ method: "GET", url: "/sessions?page=1&limit=10&sort=createdAt&order=desc" });
    expect(res.statusCode).toBe(200);
    const json = res.json();
  ...
  it("GET /projects/:id/sessions returns list", async () => {
    projectService.getProjectSessions.mockResolvedValue({ data: [exampleSession] });

    const res = await server.inject({ method: "GET", url: `/projects/${exampleProject.id}/sessions` });
    expect(res.statusCode).toBe(200);
