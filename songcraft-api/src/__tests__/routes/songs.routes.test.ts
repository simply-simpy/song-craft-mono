...
describe("songs routes", () => {
  ...
  const server = createTestServer({ register: songsRoutes, container: { songsService } });
  ...
  it("GET /songs returns paginated list", async () => {
    ...
    const res = await server.inject({ method: "GET", url: "/songs?page=1&limit=20&sort=updatedAt&order=desc" });
    expect(res.statusCode).toBe(200);
    const json = res.json();
    expect(json.songs).toHaveLength(1);
    expect(json.pagination.total).toBe(1);
    expect(songsService.listSongs).toHaveBeenCalledWith({ ownerClerkId: undefined }, { page: 1, limit: 20, sort: "updatedAt", order: "desc" });
  });

  it("GET /songs/:id returns a song", async () => {
    songsService.findById.mockResolvedValue(exampleSong);

    const res = await server.inject({ method: "GET", url: `/songs/${exampleSong.id}` });
    expect(res.statusCode).toBe(200);
...
const res = await server.inject({ method: "GET", url: "/songs/123e4567-e89b-12d3-a456-426614174001" });
  ...
  it("GET /songs/short/:shortId returns a song", async () => {
    ...
    const res = await server.inject({ method: "GET", url: `/songs/short/${exampleSong.shortId}` });
    expect(res.statusCode).toBe(200);
...
const res = await server.inject({ method: "GET", url: "/songs/short/6f0bbde763f89735" });
    expect(res.statusCode).toBe(404);
    expect(res.json().code).toBe("NOT_FOUND");
  });

  it("GET /songs/:id/versions returns versions array", async () => {
    songsService.getSongVersions.mockResolvedValue([{ id: "v1" }, { id: "v2" }]);

    const res = await server.inject({ method: "GET", url: `/songs/${exampleSong.id}/versions` });
    expect(res.statusCode).toBe(200);
    expect(res.json().versions).toHaveLength(2);
  });

  it("POST /songs creates a song when headers and body valid", async () => {
    songsService.createSong.mockResolvedValue({ ...exampleSong, title: "New Song" });
  ...
  it("PUT /songs/:id updates a song", async () => {
    songsService.updateSong.mockResolvedValue({ ...exampleSong, title: "Updated" });
