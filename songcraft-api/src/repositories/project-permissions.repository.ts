...
export interface IProjectPermissionsRepository {
	...
	findByProjectAndUser(
		projectId: string,
		userId: string,
	): Promise<DbProjectPermission | null>;
	findByProjectAndUserWithDetails(
		projectId: string,
		userId: string,
	): Promise<ProjectPermissionWithUser | null>;
	update(
		projectId: string,
		userId: string,
	...
	implements IProjectPermissionsRepository
{
	...
	private serializePermission(permission: {
		...
		return {
			...
			grantedAt: permission.grantedAt as Date,
			expiresAt: permission.expiresAt as Date | null,
			userEmail: permission.userEmail as string | null,
		};
	}

	async create(
		data: CreateProjectPermissionData,
	): Promise<DbProjectPermission> {
	...
	async findByProjectAndUserWithDetails(
		projectId: string,
		userId: string,
	): Promise<ProjectPermissionWithUser | null> {
		const [permission] = await this.db
