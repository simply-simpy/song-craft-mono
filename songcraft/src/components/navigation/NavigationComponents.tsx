...
export function NavLink({
	...
	const baseClasses =
		"inline-flex items-center font-medium transition-colors hover:underline";
		...
		<nav className={cn("flex items-center space-x-2 text-sm", className)}>
			{items.map((item, index) => (
				<React.Fragment key={item.label}>
					{index > 0 && <span className="text-fg-tertiary">{separator}</span>}
					{item.to || item.href ? (
						item.to ? (
							<Link
								to={item.to}
...
export function Tabs({ children, className }: TabsProps) {
	return (
		<div className={cn("border-b border-border-secondary", className)}>
